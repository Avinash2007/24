...
public class LightboxConfig {
  ...
  public static var loadImage: (UIImageView, URL, ((UIImage?) -> Void)?) -> Void = { (imageView, imageURL, completion) in
    imageView.sd_setImageWithPreviousCachedImage(with: imageURL, placeholderImage: nil, options: SDWebImageOptions(rawValue: 0), progress: nil) {  (image, error, imageCacheType, imageUrl) in
        if image != nil{
            completion?(image)
        }
    }
